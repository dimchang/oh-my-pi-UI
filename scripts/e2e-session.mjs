...
rl.on('line',(l)=>{const t=l.trim();if(!t)return;let f;try{f=JSON.parse(t)}catch{return}
  ...
  else if(f.type==='response'&&f.command==='get_session_stats'){console.log('[ok] get_session_stats success=',f.success);step='done';clearTimeout(dl);child.kill('SIGKILL');console.log('SESSION-CMDS PASS');process.exit(0);}
});
